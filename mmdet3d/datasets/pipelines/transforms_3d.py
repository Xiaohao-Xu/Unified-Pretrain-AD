...
class GridMask_self:
    ...
    def __call__(self, results):
        ...
        if self.offset:
            offset = torch.from_numpy(2 * (np.random.rand(h, w) - 0.5)).float()
            offset = (1 - mask) * offset
            imgs_new = [x * mask + offset for x in imgs]
        else:
            imgs_new = [x * mask for x in imgs] + [x * (1-mask) for x in imgs] #+  [torch.tensor(mask)]

        results.update(img=imgs_new)
        return results
